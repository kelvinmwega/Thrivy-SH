var getDevDataURL = ctx + "/getTrackerEMDData";
var batlevelChart, templevelChart, humlevelChart, lightChart, speedlevelChart, siglevelChart;
...
function fetchHistoricalData(){
    ...
    var mylmdevName = "865905027550908";
    ...
    try {
        ...
        for (var i = 0; i < data.docs.length; i++) {
            ...
            try {
                ...
                speedArray.push(data.docs[i].speed);
        ...
        loadSignalLevel(labelsArray, signalArray);
    }
}
...
function loadSpeedLevel(labelsArray, batteryArray){
    var btctx = document.getElementById('speedChart').getContext('2d');
    ...
    speedlevelChart = new Chart(btctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Speed (Km/hr)',
                data: batteryArray,
                backgroundColor: "rgba(10, 14, 201, 1)"
            }]
        },
        options: chartOptions
    });
...
function loadSignalLevel(labelsArray, batteryArray){
    var btctx = document.getElementById('signalChart').getContext('2d');

    if (siglevelChart) {
        siglevelChart.destroy();
    }

    siglevelChart = new Chart(btctx, {
        type: 'line',
        data: {
            labels: labelsArray,
            datasets: [{
                label: 'Signal',
                data: batteryArray,
                backgroundColor: "rgba(120, 134, 1, 51)"
            }]
        },
        options: chartOptions
    });
